...
class DatosPersonalesSerializer(serializers.ModelSerializer):
    ...
    def validate_sexo(self, value):
        """Validate sex field"""
        if value and value not in ['H', 'M']:
            raise serializers.ValidationError("El sexo debe ser 'H' (Hombre) o 'M' (Mujer).")
        return value
...
class ReconocimientoSerializer(serializers.ModelSerializer):
    ...
    def validate_tipo_reconocimiento(self, value):
        """Validate recognition type"""
        valid_types = ['Académico', 'Público', 'Privado']
...
class VentaGarageSerializer(serializers.ModelSerializer):
    ...
    def validate_estado_producto(self, value):
        ...
        valid_states = ['Bueno', 'Regular']
        if value and value not in valid_states:
            raise serializers.ValidationError(f"El estado debe ser uno de: {', '.join(valid_states)}")
        return value
    
    def validate_valor_del_bien(self, value):
