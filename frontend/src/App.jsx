...
function App() {
  ...
  const allSections = [
    ...
    { id: 'reconocimientos', label: 'Reconocimientos', hideIfEmpty: true },
    { id: 'productos_academicos', label: 'Productos Académicos', hideIfEmpty: true },
    { id: 'productos_laborales', label: 'Productos Laborales', hideIfEmpty: true },
    { id: 'referencias', label: 'Referencias Personales' },
    { id: 'ventas_garage', label: 'Venta Garage', hideIfEmpty: true }
  ];

  // Filtrar secciones disponibles basado en si tienen datos
  const availableSections = useMemo(() => {
    if (!cvData) return allSections.filter(s => !s.hideIfEmpty);
    
    const sectionHasData = (sectionId) => {
      switch(sectionId) {
        case 'reconocimientos': return cvData.reconocimientos && cvData.reconocimientos.length > 0;
        case 'productos_academicos': return cvData.productos_academicos && cvData.productos_academicos.length > 0;
        case 'productos_laborales': return cvData.productos_laborales && cvData.productos_laborales.length > 0;
        case 'ventas_garage': return cvData.ventas_garage && cvData.ventas_garage.length > 0;
        default: return true;
      }
    };

    return allSections.filter(section => !section.hideIfEmpty || sectionHasData(section.id));
  }, [cvData]);
